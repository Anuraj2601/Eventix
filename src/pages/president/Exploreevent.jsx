...
const Exploreevent = () => {
  ...
  const { name, image, date, clubName, clubImage, venue } =
    location.state || {};
  //console.log("explore events location", location.state);
  const [likes, setLikes] = useState(0);
  const [liked, setLiked] = useState(false);
    ...
    try {
      ...
      const isOcArray = response2.content
        ? response2.content.filter(
            (oc) =>
              oc.event_id == eventDetails.event_id && oc.user_id == session_id
          )
        : [];
      //console.log("is oc array ",isOcArray);
  ...
  const isRegistered = async () => {
    ...
    try {
      const response2 = await EventRegistrationService.getAllEventRegistrations(
        token
      );
      const eventRegArray = response2.content
        ? response2.content.filter(
            (eReg) =>
              eReg.event_id == eventDetails.event_id &&
              eReg.user_id == session_id
          )
        : [];
      //console.log("event reg array ",eventRegArray);
        ...
        <div className="bg-neutral-900 text-white flex flex-col flex-1 overflow-auto">
          ...
          <div className="relative h-full flex flex-col justify-center">
                ...
                >
                  {isMatchingPage() && (
                    <div className="flex flex-row gap-1 top-1">
                      ...
                      <button
                        onClick={() => handleDeleteSponsor(eventS.sponsor_id)}
                      >
                        <Link className="p-1 bg-[#AEC90A] rounded-full flex items-center justify-center hover:bg-[#9ab32f]">
                          <AiOutlineDelete size={24} />
                        </Link>
            ...
            {modalIsOpen && selectedSponsor && (
              ...
              >
                <div className="modal-content bg-black p-6 rounded-lg text-white relative text-center max-w-lg mx-auto border-2 border-[#cfed10]">
                  <button
                    onClick={() => setModalIsOpen(false)}
                    className="absolute top-4 right-4 text-[#cfed10]"
                  >
                    <AiOutlineClose size={24} />
                  </button>
                  ...
                  <p>
                    ...
                    {selectedSponsor.contact_person}
                  </p>
                </div>
              </ReactModal>
            )}
            ...
            {/* <ReactModal
              ...
              style={{
                content: {
                  backgroundColor: "black", // Modal background
                  width: "50%", // Reduced modal width
                ...
                overlay: {
                  backgroundColor: "rgba(0, 0, 0, 0.8)", // Dimmed overlay background
                },
              }}
            >
              <div>
